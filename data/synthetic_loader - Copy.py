...
class EEMFNetDataset(Dataset):
    ...
    def save_and_reinforce(self):
        if self.forced_weights is not None:
            self.saved_best_weights = self.forced_weights.copy()
            self.current_w = self.type_counts / np.sum(self.type_counts)
            self.current_w = np.exp(10 * self.forced_weights)
            self.current_w /= np.sum(self.current_w)

        else:
            self.saved_best_weights = self.current_w.copy()
        self.forced_weights = self.current_w 
        self.exploration_fail_count = 0 
        self.exploration_fail_count_g = 0
        logger.info(f"   [Strategy] 🏆 New Best! Counts: {self.type_counts.astype(int)} | Weights: {np.round(self.current_w, 3)}")

    def switch_to_exploration(self):
        if self.forced_weights is not None:
            logger.info(f"   [Strategy] 🔄 No improvement. Unfreezing weights (Exploration Mode).")
            self.forced_weights = None
            self.exploration_fail_count = 0 

    def check_and_rollback(self):
        if self.forced_weights is None and self.saved_best_weights is not None:
            self.exploration_fail_count += 1
            self.exploration_fail_count_g += 1
            logger.info(f"   [Strategy] ⚠️ Exploration failed ({self.exploration_fail_count}/{self.patience_limit}): {self.type_counts.astype(int)} | Weights: {np.round(self.current_w, 3)}")
            if self.exploration_fail_count_g >= self.patience_limit_g:
                logger.info(f"   [Strategy] 🛑 Restart .............................................")
                self.exploration_fail_count_g = 0
                self.exploration_fail_count = 0
                self.type_counts = np.array([0, 0], dtype=np.float32)
                self.forced_weights = np.array([0.5, 0.5])
            elif self.exploration_fail_count >= self.patience_limit:
                logger.info(f"   [Strategy] 🛑 Limit reached! Rolling back to PREVIOUS BEST weights.")
                self.forced_weights = self.saved_best_weights # p_weights
                self.exploration_fail_count = 0 

    def get_current_inverse_weights(self):
        inverse_counts = 1.0 / (self.type_counts + 1.0)
        if len(self.f_list) == 0: inverse_counts[2] = 0.0
        total = np.sum(inverse_counts)
        return inverse_counts / total if total > 0 else np.array([0.5, 0.5])
